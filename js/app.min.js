...
Handlebars.registerHelper("select", selectHelper),
    function(e, t) {
        e.views = {
            ...
            skills: 5,
            spells: 5
        }, e.profiles = {}, e.attributes = {}, e.skills = [], e.spells = [], e.perks =[], e.mutations = [], e.injuries = [], e.books = [], e.equipment = {}, e.enchants = {}, e.resistances = {}, e.initiatives = {}, e.points = {}, e.types = {}, e.activeProfile = {}, e.baseProfile = {
            name: "",
            title: "",
            type: "",
            warband: "",
		...
		e.getEnchantedMaxValue = function(attr) {
			var enchMaxTmp = e.activeProfile.attributes[attr].max.current;
			for (var a in e.activeProfile.enchants) {
                if (e.activeProfile.enchants[a] !== !1) {
					var r = e.activeProfile.enchants[a],
                        n = e.enchants.body[r.name] || e.enchants.head[r.name] || e.enchants.hand[r.name],
						eff = n.effects[r.version],
						regmatch = /^enchant_(\w*)$/,
						mymatch = [];
					for (var mykey in eff) {
						if (mymatch = regmatch.exec(mykey)) {
							if (mymatch[1] == attr) {
								enchMaxTmp += eff[mykey];
							}
						}
					}
				}
			}
            var enchMax = enchMaxTmp < e.activeProfile.attributes[attr].max.type ? enchMaxTmp : e.activeProfile.attributes[attr].max.type;
			return enchMax;			
		}, 
		e.getEnchantedValue = function(attr) {
			var enchMax = e.getEnchantedMaxValue(attr);
			var enchCurTmp = e.activeProfile.attributes[attr].value;
			for (var a in e.activeProfile.enchants) {
                if (e.activeProfile.enchants[a] !== !1) {
					var r = e.activeProfile.enchants[a],
                        n = e.enchants.body[r.name] || e.enchants.head[r.name] || e.enchants.hand[r.name],
					...
					for (var mykey in eff) {
						if (mymatch = regmatch.exec(mykey)) {
							if (mymatch[1] == attr) {
								enchCurTmp += eff[mykey];
							}
						}
					}
				}
			}
			var enchCur = enchCurTmp < enchMax ? enchCurTmp : enchMax;
			return enchCur;
		}, 
		e.attributeUpdate = function(i, a) {
            ...
            for (var a in e.activeProfile.attributes) {
                var r = $("#attribute-" + a);
                if (r.length) {
					var norMax = e.activeProfile.attributes[a].max.current < e.activeProfile.attributes[a].max.type ? e.activeProfile.attributes[a].max.current : e.activeProfile.attributes[a].max.type;
                    var enchMax = e.getEnchantedMaxValue(a);
					var enchCur = e.getEnchantedValue(a);
					var curText;
					var maxText;
					if (enchCur > e.activeProfile.attributes[a].value) {
						curText = enchCur + " (" + e.activeProfile.attributes[a].value + ")";
					} else {
						curText = enchCur;
					}
					if (enchMax > norMax) {
						maxText = enchMax + " (" + norMax + ")";
					} else {
						maxText = enchMax;
					}
                    r.find(".attribute-value").text(curText + " / " + maxText)
                }
            }
            ...
            e.activeProfile.restrictions.spells.length > 0 ? "none" === e.panels.spells.css("display") && e.panels.spells.toggle() : "none" !== e.panels.spells.css("display") && e.panels.spells.toggle(), e.profiles[e.activeProfile.name].restrictions.mutations !== t ? "none" === e.panels.mutations.css("display") && e.panels.mutations.toggle() : "none" !== e.panels.mutations.css("display") && e.panels.mutations.toggle(), e.activeProfile.perks.length > 0 ? "none" === e.panels.perks.css("display") && e.panels.perks.toggle() : "none" !== e.panels.perks.css("display") && e.panels.perks.toggle(), $(".ability-select option").prop("selected", !1)
        }, e.profileViewUpdate = function() {
            if (0 !== e.activeProfile.name.length) {
                e.attributeViewUpdate(), e.resistancesViewUpdate(), e.generalViewUpdate(), e.otherViewUpdate(), e.profilePanelViewUpdate(), e.uiOptionShowAll(), e.uiRestrictSkillSelect(), e.uiRestrictSpellSelect(), e.uiRestrictMutationSelect(), e.uiRestrictEquipmentSelect(), e.skillViewUpdate(), e.perksViewUpdate();
                var i = !0,
                    a = !1,
                    r = t;
                try {
                ...
                e.injuryViewUpdate();
				if (e.enchantViewUpdate(), e.uiPanelHide("enchant-head"), e.uiPanelHide("enchant-offhand"), e.uiPanelHide("enchant-mainhand"), e.uiPanelHide("enchant-body"), e.activeProfile.equipment.mainhand !== !1 && e.activeProfile.equipment.mainhand.version != "normal" && e.uiPanelShow("enchant-mainhand"), e.activeProfile.equipment.offhand !== !1 && e.activeProfile.equipment.offhand.version != "normal" &&  e.uiPanelShow("enchant-offhand"), e.activeProfile.equipment.body !== !1 && e.activeProfile.equipment.body.version != "normal" &&  e.uiPanelShow("enchant-body"), e.activeProfile.equipment.head !== !1 && e.activeProfile.equipment.head.version != "normal" &&  e.uiPanelShow("enchant-head"));
                var q = !0,
                    $ = !1,
                    x = t;
                try {
            ...
            if (r.twohanded !== t && (e.equipmentRemove("offhand") == !1), r.restrictions !== t)
			...
			if (e.activeProfile.equipment[i] === !1) {
				return !1;
			}
			if (e.activeProfile.enchants[i] !== !1) {
				if (t == "normal") {
					e.enchantRemove(i);
				} else if (t == "good") {
					e.enchantChangeVersion(t,i);
				}					
			}
			return (e.activeProfile.equipment[i].version = t, !0)
            ...
            for (var a in e.activeProfile.equipment)
                if (e.activeProfile.equipment[a] !== !1) {
                    ...
                    u.initiative > 0 && (u.initiative = "+" + u.initiative), n.effects[r.version].damage !== t && (u.damage = n.effects[r.version].damage), n.effects[r.version].armour !== t && (u.armour = n.effects[r.version].armour + "%"), n.effects.good === t && (u.quality = !1), n.effects[r.version].range !== t && (u.range = n.effects[r.version].range);
                    var f = e.templates.skill(u);
                    f = $(f), $("#equipment-" + a + "-table").find("tr").eq(-1).before(f)
                }
        }, e.enchantAdd = function(i, a) {
            if (e.enchants === t || e.enchants.body[i] === t && e.enchants.head[i] === t && e.enchants.hand[i] === t) return !1;
            var r = e.enchants.body[i] || e.enchants.head[i] || e.enchants.hand[i];
            var l = {
                name: i,
                version: "good"
            };
            return e.activeProfile.enchants[a] = l, !0
        }, e.enchantRemove = function(t) {
            return e.activeProfile.enchants[t] = !1, !0;
        }, e.enchantChangeVersion = function(t, i) {
            return e.activeProfile.enchants[i] === !1 ? !1 : (e.activeProfile.enchants[i].version = t, !0)
        }, e.enchantViewUpdate = function() {
            var i = Object.keys(e.activeProfile.enchants).map(function(e) {
                return ".enchant-" + e + "-row"
            }).join(", ");
            $(i).remove();
            for (var a in e.activeProfile.enchants)
                if (e.activeProfile.enchants[a] !== !1) {
					...
					if (e.activeProfile.equipment[a].version !== !1 && e.activeProfile.equipment[a].version == "masterwork") {
						u.enchantlevel = true;
					}
                    var f = e.templates.skill(u);
                    f = $(f), $("#enchant-" + a + "-table").find("tr").eq(-1).before(f)
                }
        },
		e.enchantGetList = function(ench_location,item_type) {
			var retval = {};
			for (var enchant_handle in e.enchants[ench_location]) {
				var enchant = e.enchants[ench_location][enchant_handle],
					title   = enchant.name;
				if (ench_location == "hand") {
					var allowed_types = enchant.restrictions;
					for (var iter = 0; iter < allowed_types.length; iter++) {
						if (allowed_types[iter] == item_type) {
							retval[enchant_handle] = title;
						}
					}
				} else {
					retval[enchant_handle] = title;
		...
		e.enchantLoadList = function(dropdown, enchant_location) {
			dropdown.empty();
			var enchantList = {};
			if (enchant_location == "head" || enchant_location == "body") {
				enchantList = e.enchantGetList(enchant_location, "");
			} else {
				var item_name = e.activeProfile.equipment[enchant_location].name;
				var my_item   = e.equipment.weapon[item_name] || e.equipment.armour[item_name];
				var my_type   = my_item.type;
				enchantList   = e.enchantGetList("hand", my_type);
			}
			dropdown.append("<option>Choose an enchantment</option>");
			for (var enchant in enchantList) {
				dropdown.append("<option value = " + enchant + ">" + enchantList[enchant] + "</option>");
			}
		}
		e.injuryHandleEffects = function(i, a) {
            if (1 === a || -1 === a) {
                if ("severed_arm" === i && (e.equipmentRemove("offhand") == !1, e.activeProfile.equipment.mainhand !== !1)) {
            ...
            try {
                for (var n, l = e.activeProfile.perks[Symbol.iterator](); !(i = (n = l.next()).done); i = !0) {
                    ...
                    var p = {
                        ...
                        v = e.templates.skill(p);
                    v = $(v), $("#perks-table").find("tbody").append(v)
                }
            } catch (m) {
                a = !0, r = m;
            } finally {
            ...
            for (var i in e.activeProfile.resistances) {
                ...
                if (a.length) {
                    ...
                    for (var loc in e.activeProfile.enchants)
                        if (e.activeProfile.enchants[loc] !== !1) {
                            var enc = e.enchants.body[e.activeProfile.enchants[loc].name] || e.enchants.head[e.activeProfile.enchants[loc].name] || e.enchants.hand[e.activeProfile.enchants[loc].name];
                            if (enc.effects[e.activeProfile.enchants[loc].version][i] !== t) {
                                var l = enc.effects[e.activeProfile.enchants[loc].version][i];
                                r += l
                            }
                        }
            ...
            for (var i in e.activeProfile.other) {
                ...
                if (a.length) {
                    ...
                    for (var loc in e.activeProfile.enchants) {
                        if (e.activeProfile.enchants[loc] !== !1) {
                            var enc = e.enchants.head[e.activeProfile.enchants[loc].name]  || e.enchants.body[e.activeProfile.enchants[loc].name] || e.enchants.hand[e.activeProfile.enchants[loc].name],
                                o = 0;
                            if (enc.effects[e.activeProfile.enchants[loc].version][i] === t) continue;
                            o = enc.effects[e.activeProfile.enchants[loc].version][i]
                            r += o
                        }
					}
            ...
            var i = $("#general-value-wounds"),
                ...
                r = 1 + e.getEnchantedValue("toughness") * e.attributes.toughness.effects.wounds / 100;
			...
			for (var loc in e.activeProfile.enchants) {
				if (e.activeProfile.enchants[loc] !== !1) {
					var enc = e.enchants.head[e.activeProfile.enchants[loc].name]  || e.enchants.body[e.activeProfile.enchants[loc].name] || e.enchants.hand[e.activeProfile.enchants[loc].name],
						o = 0;
					if (enc.effects[e.activeProfile.enchants[loc].version].wounds === t) continue;
					o = enc.effects[e.activeProfile.enchants[loc].version].wounds
					a += o
				}
			}
            ...
            if (e.activeProfile.equipment.mainhand !== !1) {
				...
				var ench_mod = 0;
				if (e.activeProfile.enchants.mainhand !== !1) {
					var enc = e.enchants.hand[e.activeProfile.enchants.mainhand.name];
					if (enc.effects[e.activeProfile.enchants.mainhand.version].damage_boost !== t) {
						ench_mod = enc.effects[e.activeProfile.enchants.mainhand.version].damage_boost;
					}
				}
                q.min += x.effects[e.activeProfile.equipment.mainhand.version].damage.min + ench_mod, q.max += x.effects[e.activeProfile.equipment.mainhand.version].damage.max + ench_mod, x.dual_wield !== t && (g = !0), "ranged" === x.type && (w = "ranged_damage")
            }
            if (e.activeProfile.equipment.offhand !== !1) {
                var x = e.equipment.weapon[e.activeProfile.equipment.offhand.name];
				var ench_mod = 0;
				if (e.activeProfile.enchants.offhand !== !1) {
					var enc = e.enchants.hand[e.activeProfile.enchants.offhand.name];
					if (enc.effects[e.activeProfile.enchants.offhand.version].damage_boost !== t) {
						ench_mod = enc.effects[e.activeProfile.enchants.offhand.version].damage_boost;
					}
				}
                x.effects[e.activeProfile.equipment.offhand.version].damage !== t && (q.min += x.effects[e.activeProfile.equipment.offhand.version].damage.min + ench_mod, q.max += x.effects[e.activeProfile.equipment.offhand.version].damage.max + ench_mod), e.activeProfile.equipment.mainhand !== !1 && "melee" == x.type && (g = !0)
            }
            if ("melee_damage" === w) {
                q.min *= 1 + e.getEnchantedValue("strength") * e.attributes.strength.effects[w] / 100, q.max *= 1 + e.getEnchantedValue("strength") * e.attributes.strength.effects[w] / 100;
                var S = !0,
                    A = !1,
                    _ = t;
                try {
            ...
            } else "ranged_damage" === w && (q.min *= 1 + e.getEnchantedValue("ballistic_skill") * e.attributes.ballistic_skill.effects[w] / 100, q.max *= 1 + e.getEnchantedValue("ballistic_skill") * e.attributes.ballistic_skill.effects[w] / 100);
            ...
            Y += e.getEnchantedValue("accuracy") * e.attributes.accuracy.effects.critical_hit_chance;
            var J = !0,
                Q = !1,
                W = t;
            try {
            ...
            var t = {
				...
				enchants: e.activeProfile.enchants,
                skills: [],
                spells: [],
                mutations: [],
                injuries: [],
			...
			e.activeProfile.enchants  = i.enchants;
            ...
            $(document).on("click", "#equipment-mainhand-action-add", function(t) {
                ...
                t.preventDefault(), e.uiAlertClear();
                var i = $(this).val();
				var mHandEnchantField = $("#enchant-mainhand-select");
                e.equipmentChangeVersion(i, "mainhand"), e.enchantLoadList(mHandEnchantField, "mainhand"), e.profileViewUpdate()
                ...
                t.preventDefault(), e.uiAlertClear();
                var i = $(this).val();
				var oHandEnchantField = $("#enchant-offhand-select");
                e.equipmentChangeVersion(i, "offhand"), e.enchantLoadList(oHandEnchantField, "offhand"), e.profileViewUpdate()
                ...
                t.preventDefault(), e.uiAlertClear();
                var i = $(this).val();
				var bodyEnchantField = $("#enchant-body-select");
                e.equipmentChangeVersion(i, "body"), e.enchantLoadList(bodyEnchantField, "body"), e.profileViewUpdate()
                ...
                t.preventDefault(), e.uiAlertClear();
                var i = $(this).val();
				var headEnchantField = $("#enchant-head-select");
                e.equipmentChangeVersion(i, "head"), e.enchantLoadList(headEnchantField, "head"), e.profileViewUpdate()
            })
        },e.uiHandlerEnchants = function() {
            $(document).on("click", "#enchant-mainhand-action-add", function(t) {
                t.preventDefault(), e.uiAlertClear();
                var i = $("#enchant-mainhand-select").val(),
                    a = e.enchantAdd(i, "mainhand");
                a && (e.profileViewUpdate(), $("#panel-enchant-mainhand .ability-select option").prop("selected", !1))
            }), $(document).on("click", ".enchant-mainhand-action-remove", function(t) {
                t.preventDefault(), e.uiAlertClear();
                var i = e.enchantRemove("mainhand");
                i && e.profileViewUpdate()
            }), $(document).on("change", ".enchant-mainhand-select-version", function(t) {
                t.preventDefault(), e.uiAlertClear();
                var i = $(this).val();
                e.enchantChangeVersion(i, "mainhand"), e.profileViewUpdate()
            }), $(document).on("click", "#enchant-offhand-action-add", function(t) {
                t.preventDefault(), e.uiAlertClear();
                var i = $("#enchant-offhand-select").val(),
                    a = e.enchantAdd(i, "offhand");
                a && (e.profileViewUpdate(), $("#panel-enchant-offhand .ability-select option").prop("selected", !1))
            }), $(document).on("click", ".enchant-offhand-action-remove", function(t) {
                t.preventDefault(), e.uiAlertClear();
                var i = e.enchantRemove("offhand");
                i && e.profileViewUpdate()
            }), $(document).on("change", ".enchant-offhand-select-version", function(t) {
                t.preventDefault(), e.uiAlertClear();
                var i = $(this).val();
                e.enchantChangeVersion(i, "offhand"), e.profileViewUpdate()
            }), $(document).on("click", "#enchant-body-action-add", function(t) {
                t.preventDefault(), e.uiAlertClear();
                var i = $("#enchant-body-select").val(),
                    a = e.enchantAdd(i, "body");
                a && (e.profileViewUpdate(), $("#panel-enchant-body .ability-select option").prop("selected", !1))
            }), $(document).on("click", ".enchant-body-action-remove", function(t) {
                t.preventDefault(), e.uiAlertClear();
                var i = e.enchantRemove("body");
                i && e.profileViewUpdate()
            }), $(document).on("change", ".enchant-body-select-version", function(t) {
                t.preventDefault(), e.uiAlertClear();
                var i = $(this).val();
                e.enchantChangeVersion(i, "body"), e.profileViewUpdate()
            }), $(document).on("click", "#enchant-head-action-add", function(t) {
                t.preventDefault(), e.uiAlertClear();
                var i = $("#enchant-head-select").val(),
                    a = e.enchantAdd(i, "head");
                a && (e.profileViewUpdate(), $("#panel-enchant-head .ability-select option").prop("selected", !1))
            }), $(document).on("click", ".enchant-head-action-remove", function(t) {
                t.preventDefault(), e.uiAlertClear();
                var i = e.enchantRemove("head");
                i && e.profileViewUpdate()
            }), $(document).on("change", ".enchant-head-select-version", function(t) {
                t.preventDefault(), e.uiAlertClear();
                var i = $(this).val();
                e.enchantChangeVersion(i, "head"), e.profileViewUpdate()
            })
            ...
            e.uiOptionShowOnly(i.mainhand, "equipment-mainhand"), e.uiOptionShowOnly(i.offhand, "equipment-offhand"), e.uiOptionShowOnly(i.head, "equipment-head"), e.uiOptionShowOnly(i.body, "equipment-body")
            ...
            for (var a in e.activeProfile.equipment)
                if (e.activeProfile.equipment[a] !== !1) {
					...
					var ench_string = "";
					if (e.activeProfile.enchants[a] !== !1) {
						var ench = e.enchants.head[e.activeProfile.enchants[a].name] || e.enchants.body[e.activeProfile.enchants[a].name] || e.enchants.hand[e.activeProfile.enchants[a].name];
						var ench_name = ench.name;
						var ench_qual = e.activeProfile.enchants[a].version == "good" ? "normal" : "master";
						ench_string   = " enc: " + ench_name + " (" + ench_qual + ")";
					}
                    i.push(r.name + " (" + qual + ")" + ench_string);
                }
            ...
            $.getJSON("data/data.json", function(t) {
                e.attributes = Object.create(t.attributes), e.skills = Object.create(t.skills), e.spells = Object.create(t.spells), e.mutations = Object.create(t.mutations), e.injuries = Object.create(t.injuries), e.books = Object.create(t.books), e.equipment = Object.create(t.equipment), e.enchants = Object.create(t.enchants), e.profiles = Object.create(t.profiles), e.resistances = Object.create(t.resistances), e.initiatives = Object.create(t.initiatives), e.points = Object.create(t.points), e.perks = Object.create(t.perks), e.types = Object.create(t.types), $(document).trigger("MCotDPlanner_DataLoaded")
            })
            ...
            e.templates.spell = $("#template-spell-row").html(), Mustache.parse(e.templates.spell), e.templates.skill.active = $("#template-skill-row-active").html(), Mustache.parse(e.templates.skill.active), e.templates.skill.passive = $("#template-skill-row-passive").html(), Mustache.parse(e.templates.skill.passive), e.templates.alert = $("#template-alert").html(), Mustache.parse(e.templates.alert), e.templates.profile = $("#template-panel-profile").html(), Mustache.parse(e.templates.profile), e.templates.perk = $("#template-perk-row").html(), Mustache.parse(e.templates.perk)
        }, e.init = function() {
            e.views.loading = $("#view-loading"), e.views.profileicons = $("#view-profileicons"), e.views.profile = $("#view-profile"), e.panels.skills = $("#panel-skills"), e.panels.spells = $("#panel-spells"), e.panels.perks = $("#panel-perks") ,e.panels.mutations = $("#panel-mutations"), e.loadData(), e.parseTemplatesHandlebars(), e.uiHandlerAttributes(), e.uiHandlerSkills(), e.uiHandlerSpells(), e.uiHandlerMutations(), e.uiHandlerInjuries(), e.uiHandlerBooks(), e.uiHandlerEquipment(), e.uiHandlerEnchants(), e.uiHandlerProfile(), e.uiHandlerDataLoaded(), e.uiHandlerButtonQuicklink(), e.uiHandlerButtonQuicksummary()
        }
